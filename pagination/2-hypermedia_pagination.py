...
class Server:
    ...
    def get_page(self, page: int = 1, page_size: int = 10) -> List[List]:
        """Locates the pagination in a dataset"""
        ...
        data = self.__dataset[ind_range[0]:ind_range[1]]
        return data

    def get_hyper(self, page: int = 1, page_size: int = 10) -> dict:
        """Provides more details about the dataset"""
        data = self.get_page(page, page_size)

        data_set = self.__dataset
        len_set = len(data_set) if data_set else 0

        total_pages = ceil(len_set / page) if data_set else 0
